...
#######CHANGEABLE CONSTANTS#########
	#Buffer size - this is the amount of data read in at a time
	#              and written out from the data files.  This
...
_start:
	...
	call  allocate_init                #initialize the allocator
...
allocate_buffer:
	...
	pushl $BUFF_SIZE                
	call  allocate                  #ask for BUFF_SIZE bytes of memory
	popl  %ebx                      #this is unused

	cmpl  $0, %eax                  #check for errors (returns 0 on error)
	jne   store_new_buffer          #continue to store_new_buffer if none found

	pushl $ERR_NO_MEM               #otherwise do an error exit
	pushl $ERR_NO_MEM_MSG
	pushl $ERR_NO_MEM_MSG_LEN
	call  error_exit

store_new_buffer:
	movl  %eax, ST_BUFF_PTR(%ebp)   #store the location of the chunk
...
read_loop_begin:
	...
	movl  ST_BUFF_PTR(%ebp), %ecx  #the location to read into
	movl  $BUFF_SIZE, %edx         #the size of the buffer
	movl  $READ, %eax
	int   $LINUX_SYSCALL
...
continue_read_loop:
	...
	pushl ST_BUFF_PTR(%ebp)        #location of the buffer
	...
	movl  ST_BUFF_PTR(%ebp), %ecx  #location of the buffer
